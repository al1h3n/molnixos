...
{ config, pkgs, ... }:
{
  programs.yazi = {
    ...
    settings = {
      manager = {
        # other manager settings...
      };
    };

    # Prepend rules so they take priority over built-ins
    # Yazi falls back to `file --mime-type` for anything not matched here.
    # The catch-all image/* rule ensures all ImageMagick formats open correctly.
    keymap = {};  # your keymaps

    # yazi.toml [mime] section via extraConfig
    extraConfig = ''
      [mime]
      prepend_rules = [
        # JPEG 2000 and other formats file(1) may misidentify
